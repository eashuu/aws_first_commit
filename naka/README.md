...
| `tools.py` | The three demo tools (`fetch_customer`, `read_attachment`, `send_summary`), fixture-backed. `fetch_customer`'s fields now include `tax`/`bank`/`voter` (GSTIN/IFSC/voter ID) alongside `kyc`, so a scripted scenario can spend the budget on tier-1-only entities — see `demo.py`. |
| `demo.py` | A scripted driver for the four one-click scenarios, run through the **real** `DisclosureGuard`/`AuditHook`/`Rehydrator` with no model call — exposed as `POST <agent-url>/demo`. Exists because Bedrock, Comprehend and Textract are gated on a new AWS account pending verification (see `11-written-submission.md`), so an agent-loop demo alone cannot run; the guardrail itself needs no model, only a fixed call sequence in place of the model choosing one. Every row it writes is real and is marked `tiers_unavailable: ["comprehend"]` (`degraded_ok=True` on the guard) — a narrower scan is always labelled as one, never presented as "scanned clean". |
...
| `selfcheck.py` | `python selfcheck.py` — 76 assertions on the logic that doesn't need AWS to get right (Verhoeff correctness incl. the Luhn-discriminating adjacent-pair-transposition test, offset-preserving redaction, rehydration round-trips, ledger accumulation, the exact Cedar decisions the demo beat sheet requires, audit-row leak safety, shadow-mode parity and non-interference). All pass. |
